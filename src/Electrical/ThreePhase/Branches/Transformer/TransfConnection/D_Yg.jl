...
#
# OpenIPSL 3.1.0 - Electrical/ThreePhase/Branches/Transformer/TransfConnection/D_Yg.mo (function)
# The 12x12 pi matrix [A B; C D] of a D_Yg three-phase transformer, argument order (X, R, tap) as in the .mo.
# Named TransfConnection_D_Yg in Julia: the nine connections repeat the names of the MonoTri TransformerFcn family
...
function TransfConnection_D_Yg(X, R, tap)
    ...
    zero = 1e-6
    Y_ser1 = [sGser -sBser zero zero mGser -mBser;
    ...
    Y_ser2 = [sGser -sBser mGser -mBser zero zero;
    ...
    Yshtm = [sGshm -sBshm mGshm -mBshm zero zero;
             sBshm sGshm mBshm mGshm zero zero;
             zero zero sGshm -sBshm mGshm -mBshm;
             zero zero sBshm sGshm mBshm mGshm;
             mGshm -mBshm zero zero sGshm -sBshm;
             mBshm mGshm zero zero sBshm sGshm]
    A = Y_ser1 + Yshtk
    B = -Y_ser1
    C = -Y_ser2
    D = Y_ser2 + Yshtm
