...
#
# OpenIPSL 3.1.0 - Electrical/ThreePhase/Loads/Dyn_wye_2Ph_unbalanced.mo (extends ThreePhase/ThreePhaseComponent.mo)
# Variable unbalanced two-phase wye load: each phase follows its own element of the two input vectors.
...
# `P_in` and `Q_in` are declared `RealInput[2]` in the `.mo`: array variables indexed after `getproperty`
# (rule 6.5), which is what the transcribed Test connects element by element.

@component function Dyn_wye_2Ph_unbalanced(; name, S_b = 100e6, fn = 50, P0_a, Q0_a, P0_b, Q0_b)
    @named base = ThreePhaseComponent(; S_b)
    @unpack S_p = base
    P0_a, Q0_a, P0_b, Q0_b = float.((P0_a, Q0_a, P0_b, Q0_b))   # F-21
    n = (; P0_a, Q0_a, P0_b, Q0_b)
    ...
    vars = @variables begin
        (P_in(t))[1:2], [description = "External P (pu of the phase base)"]
        (Q_in(t))[1:2], [description = "External Q (pu of the phase base)"]
