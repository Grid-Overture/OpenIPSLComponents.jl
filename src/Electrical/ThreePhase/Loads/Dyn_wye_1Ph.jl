...
#
# OpenIPSL 3.1.0 - Electrical/ThreePhase/Loads/Dyn_wye_1Ph.mo (extends ThreePhase/ThreePhaseComponent.mo)
# Variable single-phase wye load: the power consumed follows two real inputs.
...
# `P_in` per unit and the system base never appears (sic). `P_a` and `Q_a` are `protected` in the `.mo`
...
@component function Dyn_wye_1Ph(; name, S_b = 100e6, fn = 50, P0, Q0)
    ...
    vars = @variables begin
        P_in(t), [description = "External P (pu of the phase base)"]
        Q_in(t), [description = "External Q (pu of the phase base)"]
    ...
    # P_a(start = P0), Q_a(start = Q0), ...
    extend(System(eqs, t, vars, pars; name, systems,
        guesses = Dict(P_a => n.P0,
            Q_a => n.Q0)), base)
end
