...
#
# OpenIPSL 3.1.0 - Electrical/Machines/PSSE/GENROE.mo (extends BaseClasses/baseMachine.mo)
# GENROU with an exponential saturation (SE_exp instead of SE). Two other differences from GENROU.mo, both replicated:
# the extends modifier also starts EFD at efd0 and PMECH at pm0, and `PSIq0 = PSIppq0 - Xppq*iq0` has the opposite
# sign of GENROU's `(-PSIppq0) - Xppq*iq0` (a start value only, the equations are identical). Same structure as
# GENROU.jl: complex initialization before `@parameters`, numeric saturation coefficients into SE_exp, the four
# `initial equation der(.) = 0` as `initialization_eqs`. Omitted: Icons.VerifiedModel, graphical annotations.

@component function GENROE(; name, S_b = 100e6, V_b = 400e3, fn = 50, P_0 = 1e6, Q_0 = 0, v_0 = 1, angle_0 = 0,
    ...
    PSIq0 = PSIppq0 - Xppq * iq0   # GENROU.mo has (-PSIppq0) - Xppq*iq0 here; the .mo difference is replicated
    ...
    pars = @parameters begin
        Xpq = Xpq, [description = "Sub-transient reactance (pu)"]
    ...
    eqs = Equation[
        ...
        XadIfd ~ K1d * (Epq - PSIkd - (Xpd - Xl) * id) + Epq + id * (Xd - Xpd) + SE_exp(PSIpp, sat...) * PSIppd,
        XaqIlq ~ K1q * (Epd - PSIkq + (Xpq - Xl) * iq) + Epd - iq * (Xq - Xpq) -
                 SE_exp(PSIpp, sat...) * (-1) * PSIppq * (Xq - Xl) / (Xd - Xl),
        ud ~ (-PSIq) - R_a * id,
        uq ~ PSId - R_a * iq,
    ]
    extend(System(eqs, t, vars, pars; name,
            ...
            guesses = Dict(EFD => efd0, XADIFD => efd0, PMECH => pm0, id => id0, iq => iq0, ud => ud0, uq => uq0,
                Te => pm0, Epd => Epd0, Epq => Epq0, PSIkd => PSIkd0, PSIkq => PSIkq0, PSId => PSId0, PSIq => PSIq0,
