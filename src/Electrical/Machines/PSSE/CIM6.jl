...
#
# OpenIPSL 3.1.0 - Electrical/Machines/PSSE/CIM6.mo (extends BaseClasses/baseMotor.mo)
...
# The twin of CIM5.jl: the same electrical model, the same parameter branches and the same 24 `EQ*`; what CIM6
# adds is the general load-torque characteristic `TL = T_nom*(A*w^2 + B*w + C0 + D*w^E)` in place of CIM5's
# `TL = T_nom*(1 - s)^D`. Note that the shipped defaults make it `TL = 4` pu at rated speed, four times the motor
# rating, so no operating point exists near w = 1 (F-90): they are reproduced, not fixed.
# Omitted: graphical annotations, the commented-out P/Q alternatives of the .mo.

@component function CIM6(; name, S_b = 100e6, V_b = 400e3, fn = 50, P_0 = 1e6, Q_0 = 0, v_0 = 1, angle_0 = 0,
        M_b = 15e6, Sup = true, Ctrl = true, N = 1, H = 0.4, Mtype = 1, Ra = 0, Xa = 0.0759, Xm = 3.1241,
        R1 = 0.0085, X1 = 0.0759, R2 = 0, X2 = 0, E1 = 1, SE1 = 0.06, E2 = 1.2, SE2 = 0.6, T_nom = 1, A = 1,
        B = 1, D = 1, E = 1, C0 = 1)
    Ra, Xa, Xm, R1, X1, R2, X2, E1, SE1, E2, SE2, T_nom, A, B, D, E, C0 =
        float.((Ra, Xa, Xm, R1, X1, R2, X2, E1, SE1, E2, SE2, T_nom, A, B, D, E, C0))   # F-21
    ...
    pars = @parameters begin
        ...
        A = A, [description = "Load Torque Coefficient A"]
        B = B, [description = "Load Torque Coefficient B"]
        D = D, [description = "Load Torque Coefficient D"]
        E = E, [description = "Load Torque Coefficient E"]
        C0 = C0, [description = "Load Torque Coefficient C0"]
    end
