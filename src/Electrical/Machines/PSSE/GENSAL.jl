...
#
# OpenIPSL 3.1.0 - Electrical/Machines/PSSE/GENSAL.mo (extends BaseClasses/baseMachine.mo)
# Salient pole generator, quadratic saturation on the d-axis. The protected `Complex` chain (Zs, VT, S, It, Is,
# PSIpp0, a, Epqp, DQ_dq, I_dq, PSIpp0_dq) is Julia complex arithmetic before `@parameters`; `SE` gets the numeric
# saturation coefficients and a symbolic argument (F-22). Three states (Epq, PSIkd, PSIppq) with their literal
# `initial equation der(.) = 0` as `initialization_eqs` (F-26), `delta(start = delta0, fixed = true)` from the
# extends modifier. Xpq and Tpq0 do not exist here: the q-axis has no transient winding. CoB, p0, q0, vr0, vi0, ir0,
# ii0 come from the base. Omitted: Icons.VerifiedModel, graphical annotations.

@component function GENSAL(; name, S_b = 100e6, V_b = 400e3, fn = 50, P_0 = 1e6, Q_0 = 0, v_0 = 1, angle_0 = 0,
    ...
    pars = @parameters begin
        delta0 = delta0, [description = "rotor angle in radians"]
    ...
    eqs = Equation[
        ...
        PSId ~ PSIppd - Xppd * id,
        PSIq ~ (-PSIppq) - Xppq * iq,
        XadIfd ~ K1d * (Epq - PSIkd - (Xpd - Xl) * id) + (Xd - Xpd) * id + (SE(Epq, sat...) + 1) * Epq,
        Te ~ PSId * iq - PSIq * id,
        ud ~ (-PSIq) - R_a * id,
        uq ~ PSId - R_a * iq,
    ]
