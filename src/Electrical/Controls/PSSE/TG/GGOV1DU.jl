...
#
# OpenIPSL 3.1.0 - Electrical/Controls/PSSE/TG/GGOV1DU.mo (extends nothing: its ports are its own)
...
# set = Constant(Ldref), min_select = Min_select(frs0 = fsr0, nu = 3), gGOV1_Temp = LoadLimiterDU (which also takes Vmax/Vmin),
# V = Limiter(Vmax, Vmin), gGOV1_Turb = GGOV1_Turbine, gGOV1_Accel = AccelerationLimiter,
# gGOV1_Power = PIDGovernorDU.
...
# Twin of GGOV1.jl: same ports, same parameters (only the default of `Teng` changes, 0 instead of
# Modelica.Constants.eps), same graph, and the two DU variants of the governor and the load limiter.
# Omitted: Icons.VerifiedModel, graphical annotations.

@component function GGOV1DU(; name, Rselect = 1, Flag = 1, delay = :FixedDelay, R = 0.04, T_pelec = 1, maxerr = 0.05,
        minerr = -0.05, Kpgov = 10, Kigov = 2, Kdgov = 0, Tdgov = 1, Vmax = 1, Vmin = 0.15, Tact = 0.5, Kturb = 1.5,
        Wfnl = 0.2, Tb = 0.1, Tc = 0, Teng = 0, Tfload = 3, Kpload = 2, Kiload = 0.67,
    ...
    systems = @named begin
        ...
        gGOV1_Temp = LoadLimiterDU(; Kturb = n.Kturb, Kpload = n.Kpload, Kiload = n.Kiload, Vmax = n.Vmax,
            Vmin = n.Vmin, Dm = n.Dm, Wfnl = n.Wfnl)
        ...
        gGOV1_Power = PIDGovernorDU(; Rselect = Rselectn, R = n.R, T_pelec = n.T_pelec, maxerr = n.maxerr, minerr = n.minerr,
