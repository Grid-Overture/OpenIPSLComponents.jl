...
#
# OpenIPSL 3.1.0 - Electrical/Controls/PSSE/ES/ESDC2A.mo (extends ES/BaseClasses/BaseExciter.mo)
# Blocks: imDerivativeLag = Derivative(K_F, T_F1, y_start = 0, InitialOutput, x_start = Efd0), hV_GATE = HV_GATE,
# imLeadLag = LeadLag(K = 1, T_C, T_B, y_start = V_R0/K_A), add3_1 = Add3(k3 = -1), rotatingExciterLimited =
# RotatingExciterLimited(T_E, E_1, E_2, S_EE_1, S_EE_2, Efd0, K_E = K_E0), TransducerDelay = SimpleLag(K = 1, T_R,
# y_start = ECOMP0), DiffV1 = Add, simpleLagLimVar = SimpleLagLimVar(K_A, T_A, y_start = V_R0), gain = Gain(V_RMIN0),
# gain1 = Gain(V_RMAX0); the port VT is a plain variable. The causal connects are equalities. The five protected
# `fixed = false` parameters are resolved from inputs (F-33): V_RMAX0/V_RMIN0 feed the two gains symbolically.
# Omitted: Icons.VerifiedModel, graphical annotations.

@component function ESDC2A(; name, T_R = 0, K_A = 400, T_A = 0.02, T_B = 0, T_C = 0, V_RMAX = 9, V_RMIN = -5.43,
    ...
    systems = @named begin
        ...
        DiffV1 = Add()
        simpleLagLimVar = SimpleLagLimVar(; K = n.K_A, T = n.T_A, y_start = V_R0)
        gain = Gain(; k = V_RMIN0)
        gain1 = Gain(; k = V_RMAX0)
    end
    ...
    eqs = Equation[
        ...
        simpleLagLimVar.u ~ hV_GATE.y,                   # connect(simpleLagLimVar.u, hV_GATE.y)
