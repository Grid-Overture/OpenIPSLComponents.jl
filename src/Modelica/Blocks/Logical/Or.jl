...
#
# Modelica Standard Library 4.0.0 - Blocks/Logical.mo, block Or
# Ports are plain variables (u1, u2, y (partialBooleanSI2SO)). Omitted: graphical annotations.

# Boolean signals are Real 0/1 in this port (PLAN-01): y = u1 or u2 is max(u1, u2).
@component function Or(; name)
    ...
    System(Equation[y ~ max(u1, u2)], t, vars, []; name)
end
