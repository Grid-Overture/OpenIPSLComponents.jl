...
#
# OpenIPSL 3.1.0 - Examples/Microgrids/UniversityCampus/CampusA/GenerationGroups/STG2/STG2MachineComplete.mo (extends OpenIPSL.Interfaces.Generator)
# Generation unit STG2 of `CampusGridA`: a `GENROU` with its excitation system, its turbine governor and a
# `DisabledPSS`.
...
# `guData(redeclare record GUnitDynamics = DynParamRecords.STG2)` is fixed in the .mo itself, not a keyword of the
# unit: it is the constant `guData = (; guDynamics = CampusA_STG2)` here, so the `.mo`'s own
...
@component function STG2MachineComplete(; name, S_b = 100e6, V_b = 400e3, fn = 50, P_0 = 1e6, Q_0 = 0, v_0 = 1, angle_0 = 0)
    # `guData(redeclare record GUnitDynamics = DynParamRecords.STG2)` of the .mo, so that every
    # parameter below reads `guData.guDynamics.<sub>.<field>` exactly as the .mo writes it
    guData = (; guDynamics = CampusA_STG2)
    ...
    systems = @named begin
        ...
        exciter = AC7B(; T_R = guData.guDynamics.excSystem.T_R, K_PR = guData.guDynamics.excSystem.K_PR, K_IR = guData.guDynamics.excSystem.K_IR, K_DR = guData.guDynamics.excSystem.K_DR, T_DR = guData.guDynamics.excSystem.T_DR, V_RMAX = guData.guDynamics.excSystem.V_RMAX, V_RMIN = guData.guDynamics.excSystem.V_RMIN, K_PA = guData.guDynamics.excSystem.K_PA, K_IA = guData.guDynamics.excSystem.K_IA, VA_MIN = guData.guDynamics.excSystem.V_AMIN, VA_MAX = guData.guDynamics.excSystem.V_AMAX, K_P = guData.guDynamics.excSystem.K_P, K_L = guData.guDynamics.excSystem.K_L, T_E = guData.guDynamics.excSystem.T_E, K_C = guData.guDynamics.excSystem.K_C, K_D = guData.guDynamics.excSystem.K_D, K_E = guData.guDynamics.excSystem.K_E, K_F1 = guData.guDynamics.excSystem.K_F1, K_F2 = guData.guDynamics.excSystem.K_F2, K_F3 = guData.guDynamics.excSystem.K_F3, T_F3 = guData.guDynamics.excSystem.T_F3, VE_MIN = guData.guDynamics.excSystem.V_EMIN, VFE_MAX = guData.guDynamics.excSystem.V_FEMAX, E_1 = guData.guDynamics.excSystem.E_1, S_EE_1 = guData.guDynamics.excSystem.S_EE_1, E_2 = guData.guDynamics.excSystem.E_2, S_EE_2 = guData.guDynamics.excSystem.S_EE_2)
        const_ = Constant(; k = 0)
