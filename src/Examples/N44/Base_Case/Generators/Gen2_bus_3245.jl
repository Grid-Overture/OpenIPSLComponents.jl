...
#
# OpenIPSL 3.1.0 - Examples/N44/Base_Case/Generators/Gen2_bus_3245.mo
# (extends Electrical/Essentials/pfComponent.mo), drafted automatically
# (2026-09-21, --kind base); reviewed by hand.
# Generation unit of bus 3245 of the Nordic 44 base case: GENSAL + HYGOV + SCRX, rated
# M_b = 950 MVA on the system's S_b = 1000 MVA, with `cte(k = 0)` on VOTHSG, VUEL and VOEL of the SCRX.
...
# Quirk of the .mo dropped: `connect(p, p)`, the pin connected to itself -- a no-op
# in Modelica, and not an MTK equation.
# Omitted: graphical annotations, displayPF.

@component function Gen2_bus_3245(; name, S_b = 100e6, V_b = 400e3, fn = 50, P_0 = 1e6, Q_0 = 0,
        v_0 = 1, angle_0 = 0, mods = (;))
    @named base = pfComponent(; S_b, V_b, fn, P_0, Q_0, v_0, angle_0)
    gENSAL = redeclared(mods, :gENSAL, GENSAL)(; name = :gENSAL, modified(mods, :gENSAL, (; Tpd0 = 5.0, Tppd0 = 0.06,
        Tppq0 = 0.1, H = 3.3, D = 0.0, Xd = 0.75, Xq = 0.5, Xpd = 0.25, Xppd = 0.15385, Xppq = 0.15385, Xl = 0.11538,
        M_b = 950*1e6, S10 = 0.10239, S12 = 0.2742, R_a = 0.0, V_b = V_b, v_0 = v_0, angle_0 = angle_0, P_0 = P_0,
        Q_0 = Q_0, S_b, fn))...)
    hYGOV = redeclared(mods, :hYGOV, HYGOV)(; name = :hYGOV, modified(mods, :hYGOV, (; R = 0.06, r = 0.4, VELM = 0.1,
        T_r = 5.0, T_f = 0.05, T_g = 0.2, G_MAX = 1.0, G_MIN = 0.0, T_w = 1.0, A_t = 1.01, D_turb = 0.5,
        q_NL = 0.1))...)
    sCRX = redeclared(mods, :sCRX, SCRX)(; name = :sCRX, modified(mods, :sCRX, (; K = 31.0, T_AT_B = 0.25385,
    ...
    systems = [gENSAL, hYGOV, sCRX, cte, p]
    eqs = Equation[
        connect(gENSAL.p, p),
        sCRX.EFD ~ gENSAL.EFD,   # connect(sCRX.EFD, gENSAL.EFD)
        cte.y ~ sCRX.VOTHSG,   # connect(cte.y, sCRX.VOTHSG)
