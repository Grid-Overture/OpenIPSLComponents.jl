...
#
# PortTests/Controls/PSSE/PSS/STABNI.mo (a Test of this port, not OpenIPSL's: PLAN-12, family B),
...
# The rig of the IEE2ST Test with the type-NI stabilizer, whose two cascaded Derivative blocks see the second
# derivative of the filtered power: PELEC <- gENROE.PELEC, VOTHSG -> eSST1A.VOTHSG. Every parameter is a default
# of the .mo. `STABNI` is the model, so the Test function takes the suffix _Test (PLAN-02 rule).
@component function STABNI_Test(; name, S_b = 100e6, fn = 50)
    ...
    eqs = Equation[
        ...
        sTABNI.PELEC ~ gENROE.PELEC,   # connect(sTABNI.PELEC, gENROE.PELEC)
        eSST1A.VOTHSG2 ~ zero.y,   # connect(eSST1A.VOTHSG2, zero.y)
        sTABNI.VOTHSG ~ eSST1A.VOTHSG,   # connect(sTABNI.VOTHSG, eSST1A.VOTHSG)
...
@testset "PortTests.Controls.PSSE.PSS.STABNI" begin
    oracle = include(joinpath(@__DIR__, "..", "..", "..", "..", "oracle", "Controls.PSSE.PSS.STABNI.jl"))
    validate_against_oracle(STABNI_Test, oracle)
end
